...
class Register(DatabaseInitializer):
    ...
    def registrate_into_team(self, student):
        insert_dt = f"INSERT INTO Student_db (name, age, type_m, is_graduated, sit_pay_curse, start_curse, finish_curse)"
        " VALUES ('{student.name}','{student.age}','{student.type_m}','{student.is_graduated}','{student.sit_pay_curse}','{student.start_curse}'"
        ",'{student.finish_curse}')"
...
class Creator(DatabaseInitializer):
    ...
    def create_table(self, team):
        insert_dt = f"create table tal {team.team_id}"
        self.cursor.execute(insert_dt)
        self.conexao.commit()
